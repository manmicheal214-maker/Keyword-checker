...
function find(rows, d) {
  for (let i = 0; i < rows.length; i++) {
    if (rows[i]?.link && match(rows[i].link, d)) return { position: i + 1, url: rows[i].link };
  }
...
function mainDoc(s) {
  ...
  x.country = s.country;
  x.country_code = s.country_code;
  x.language = s.language;
  x.search_engine = s.search_engine;
  x.max_position = Number(s.max_position) || 10;
  return x;
}
